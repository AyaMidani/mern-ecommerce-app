    ...
    <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
            ...
            <div className="flex items-center mt-1 gap-2">
                <Button variant="outline" className="h-8 w-8 rounded-full" size="icon" disabled={cartItem?.quantity === 1} onClick={()=>handleUpdateQuantity(cartItem,'minus')}>
                    <Minus className="w-4 h-4"/>
                    <span className="sr-only">Decrease</span>
                </Button>
                <span className="font-semibold">{cartItem.quantity}</span>
                 <Button variant="outline" className="h-8 w-8 rounded-full" size="icon" onClick={()=>handleUpdateQuantity(cartItem,'add')}>
        ...
        <div className="flex flex-col items-end">
            ...
            <Trash onClick={()=>handleCardItemDelete(cartItem)} className="cursor-pointer mt-1" size={20} />
        </div>
    </div>
);
}
