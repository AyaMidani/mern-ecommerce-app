...
function ProductDetailsDialog({open,setOpen,productDetails}){
    ...
    function handleAddReview(){
        dispatch(addReview({
            ...
            if(data?.payload?.success){
                ...
                toast({
                    title: 'You cannot Review this item.',
                    variant: 'destructive'
                }) 
            }
        })
    ...
    const averageReview = reviews.length && reviews.length > 0 ?
        ...
        <Dialog open={open} onOpenChange={handleDialogClose}>
            <DialogContent className="grid grid-cols-2 gap-8 sm:p-12 max-w-[90vw] sm:max-w-[80vw] lg:max-w-[70vw]">
                ...
                <div className="mt-5 mb-5">
                    {
                         Number(productDetails?.totalStock) === 0 ? <Button disabled className="w-full opacity-60 cursor-not-allowed">Out Of Stock</Button> 
