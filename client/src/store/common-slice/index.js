...
export const getFeatureImages= createAsyncThunk('common/getFeatureImages',
  async()=>{
    const response= await axios.get(`http://localhost:5001/api/common/feature/get`);
...
export const addFeatureImage= createAsyncThunk('common/addFeatureImage',
  async(image)=>{
    const response= await axios.post(`http://localhost:5001/api/common/feature/add`,{image});
    return response?.data;
  }
) 
